...
{
    private $path = './application/install/data/';
    //安装开始
    ...
    public function steps1(){
        ...
        $items = [
            ...
            ['dir',  '可写', 'ok', './runtime'],
            ['dir',  '可写', 'ok', './plugs'],
            ['file', '可写', 'ok', './application/database.php'],
            ['file', '可写', 'ok', './application/config.php'],
        ];
    ...
    public function steps3(){
        ...
        file_put_contents('./application/config.php',$config);
        ...
        file_put_contents('./application/tags.php',$tags);
        ...
        $sql = str_replace('t_',$info['prefix'],$sql);
        $sql = str_replace("\r", "\n", $sql);
        $sql = explode(";\n", $sql);
//        $db = Db::connect("mysql://{$info['username']}:{$info['password']}@{$info['ip']}:{$info['port']}/{$info['mysqlname']}");
        ...
        $regtime = time();
        $sql = "INSERT INTO `t_user` VALUES ('1', '{$username}', '{$password}', '{$regtime}', '', '{$regtime}', '0', '{$regtime}', '1')";
        Db::execute($sql);
